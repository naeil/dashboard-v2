 ...
 * getHistory() - 전체 직원 조회, created_by 필터 없음
 *             - dashboard_user LEFT JOIN으로 등록자 실명 포함
 *             - promotion_margin_form LEFT JOIN으로 프로모션명 포함
 */
@Service
...
public class PromotionMarginService {
    ...
    public Long submitForm(Long formId, Long companyId, String username) {
        ...
        BigDecimal unitProfit = salePrice.subtract(discountAmount)
                .subtract(cogs).subtract(logisticsCost).subtract(marketingCost)
                .subtract(platformFee).subtract(otherCost);
        BigDecimal targetRevenue  = salePrice.subtract(discountAmount).multiply(BigDecimal.valueOf(targetQty));
        ...
        MapSqlParameterSource hp = new MapSqlParameterSource()
                .addValue("companyId", companyId).addValue("formId", formId)
                .addValue("createdBy", username)
                ...
                .addValue("targetRevenue", targetRevenue).addValue("targetQty", targetQty)
    ...
    // fix: show all promotion records across users
    // NO created_by filter - all users data returned
    // LEFT JOIN dashboard_user for display_name (createdByName)
    // LEFT JOIN promotion_margin_form for form_name (promoName)
    @Transactional(readOnly = true)
    public List<PromotionHistoryDTO.ChannelSummary> getHistory(Long companyId, String channel) {
        String sql = """
                SELECT ph.id, ph.company_id, ph.form_id, ph.created_by,
                       COALESCE(du.display_name, ph.created_by) AS created_by_name,
                       COALESCE(pmf.form_name, '') AS promo_name,
                       ph.channel, ph.promotion_type, ph.product_name, ph.sku_code,
                       ph.target_revenue, ph.target_qty, ph.target_operating_profit,
                       ph.actual_qty, ph.actual_revenue,
                       ph.actual_cogs, ph.actual_logistics, ph.actual_marketing,
                       ph.actual_platform_fee, ph.actual_other_cost,
                       ph.actual_operating_profit, ph.revenue_achievement_rate,
                       ph.promo_start_date, ph.promo_end_date,
                       ph.submitted_at, ph.last_synced_at, ph.memo,
                       ph.created_at, ph.updated_at
                FROM promotion_history ph
                LEFT JOIN dashboard_user du ON du.username = ph.created_by
                LEFT JOIN promotion_margin_form pmf ON pmf.id = ph.form_id
                WHERE ph.company_id = :companyId
                  AND (:channel IS NULL OR ph.channel = :channel)
                ORDER BY ph.promo_start_date DESC
                """;
        ...
        List<PromotionHistoryDTO.Response> items = rows.stream()
                .map(this::mapHistoryRow).collect(Collectors.toList());
        ...
        return grouped.entrySet().stream().map(e -> {
            ...
            BigDecimal totalTarget = list.stream().map(r -> r.getTargetRevenue() != null ? r.getTargetRevenue() : BigDecimal.ZERO).reduce(BigDecimal.ZERO, BigDecimal::add);
            BigDecimal totalActual = list.stream().map(r -> r.getActualRevenue() != null ? r.getActualRevenue() : BigDecimal.ZERO).reduce(BigDecimal.ZERO, BigDecimal::add);
            BigDecimal totalProfit = list.stream().map(r -> r.getActualOperatingProfit() != null ? r.getActualOperatingProfit() : BigDecimal.ZERO).reduce(BigDecimal.ZERO, BigDecimal::add);
            ...
            return PromotionHistoryDTO.ChannelSummary.builder()
                    .channel(e.getKey()).promotionCount(list.size())
                    .totalTargetRevenue(totalTarget).totalActualRevenue(totalActual)
                    .totalActualOperatingProfit(totalProfit).overallAchievementRate(achieveRate)
                    .items(list).build();
    ...
    private PromotionHistoryDTO.Response mapHistoryRow(Map<String, Object> r) {
        return PromotionHistoryDTO.Response.builder()
                ...
                .createdByName((String) r.get("created_by_name"))
                .promoName((String) r.get("promo_name"))
    ...
    private int toInt(Object v) { if (v == null) return 0; return ((Number) v).intValue(); }
