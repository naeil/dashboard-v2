...
function loadOfflineCols() {
  try {
    ...
    return OFFLINE_COLS_DEFAULT.map(col => ({
      ...col,
      label: saved[col.key] || col.label,
    }))
  } catch {
    return [...OFFLINE_COLS_DEFAULT]
  }
}

function saveOfflineColLabel(key, label) {
  try {
    const saved = JSON.parse(localStorage.getItem(OFFLINE_COL_STORAGE_KEY) || '{}')
    saved[key] = label
    localStorage.setItem(OFFLINE_COL_STORAGE_KEY, JSON.stringify(saved))
  } catch {}
}
...
const EMPTY_OFFLINE_ROW = {
  product_code: '', product_name: '', sku_code: '',
  buyer_country: '', buyer_name: '', trade_channel: 'offline',
  trade_terms: 'EXW', currency: 'KRW', exchange_rate: 1,
  list_price: 0, production_cost: 0, consumer_ship_fee: 0,
  channel_fee_rate: 0, storage_fee_unit: 0,
  marketing_rate: 0, ad_rate: 0, opex_rate: 0,
}
...
// ── 컨럼명 편집 헤더 ──
function EditableHeader({ col, onRename }) {
  const [editing, setEditing] = useState(false)
  ...
  const commit = () => {
    setEditing(false)
    if (draft.trim()) onRename(col.key, draft.trim())
  }
  ...
  if (editing) {
    return (
      <input
        ...
        onBlur={commit}
        onKeyDown={e => { if (e.key === 'Enter') commit(); if (e.key === 'Escape') setEditing(false) }}
        style={{ width: '90%', padding: '2px 4px', fontSize: 11, background: '#0d2035', border: '1px solid #4a8fcf', borderRadius: 3, color: '#cce0f5', outline: 'none' }}
      />
    )
  }
  ...
  return (
    <span
      title="더블클릭하면 컨럼명을 수정할 수 있습니다"
      onDoubleClick={startEdit}
      style={{ cursor: 'pointer', userSelect: 'none' }}
    >
      {col.label} <span style={{ fontSize: 9, color: '#4a6a8a', marginLeft: 2 }}>✏️</span>
    </span>
  )
}

// ── 오프라인(납품처별) 전용 테이블 ──
function OfflineTable({ rows, onRefresh, notify }) {
  const [offlineCols, setOfflineCols] = useState(() => loadOfflineCols())
  ...
  const handleRename = (key, label) => {
    ...
    setOfflineCols(prev => prev.map(c => c.key === key ? { ...c, label } : c))
  }
  ...
  const handleAddSave = async () => {
    if (!newRow.product_name) {
      notify('제품명은 필수입니다.', 'error'); return
    }
    setSaving(true)
    try {
  ...
  const filteredRows = tableRows.filter(row => {
    const ps = searchProduct.toLowerCase()
    const bs = searchBuyer.toLowerCase()
    const matchProduct = !ps || String(row.product_name || '').toLowerCase().includes(ps) || String(row.product_code || '').toLowerCase().includes(ps)
    const matchBuyer = !bs || String(row.buyer_name || '').toLowerCase().includes(bs) || String(row.buyer_country || '').toLowerCase().includes(bs)
    return matchProduct && matchBuyer
  })

  const displayCols = [...offlineCols, ...CALCULATED_COLS]
    ...
    <div>
      ...
      <div style={{ display: 'flex', gap: 10, marginBottom: 14, flexWrap: 'wrap', alignItems: 'center' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 6, background: '#0d2035', border: '1px solid #2a4060', borderRadius: 6, padding: '6px 10px' }}>
          <span style={{ fontSize: 13 }}>🌏</span>
          <input
            placeholder="거래처 검색 (국가 / 바이어명)"
            value={searchBuyer}
            onChange={e => setSearchBuyer(e.target.value)}
            style={{ background: 'none', border: 'none', outline: 'none', color: '#cce0f5', fontSize: 13, width: 200 }}
          />
        </div>
        <span style={{ fontSize: 11, color: '#4a7a9a' }}>전체 {tableRows.length}건 / 필터 {filteredRows.length}건 | 컨럼명은 더블클릭으로 수정</span>
      </div>
      ...
      <div style={{ overflowX: 'auto' }}>
            ...
            <tr style={{ background: '#1a3050', color: '#8ab4d8' }}>
              {offlineCols.map(c => (
                <th key={c.key} style={{ padding: '6px 8px', textAlign: 'left', whiteSpace: 'nowrap', width: c.width, borderBottom: '1px solid #2a4060' }}>
                  <EditableHeader col={c} onRename={handleRename} />
                </th>
              ))}
              {CALCULATED_COLS.map(c => (
                <th key={c.key} style={{ padding: '6px 8px', textAlign: 'right', whiteSpace: 'nowrap', width: c.width, borderBottom: '1px solid #2a4060' }}>{c.label}</th>
              ))}
              ...
              <tr key={row.id} style={{ borderBottom: '1px solid #1e3050' }}
                ...
                {offlineCols.map(c => (
                  <td key={c.key} style={{ padding: '4px 8px', whiteSpace: 'nowrap' }}>
                    <EditCell value={row[c.key]} type={c.type} width={c.width} onSave={(val) => handleCellSave(row, c.key, val)} />
                  </td>
                ))}
                {CALCULATED_COLS.map(c => (
                  <td key={c.key} className={'calculated-profit ' + (c.tone ? c.tone(row) : '')} style={{ padding: '4px 8px', whiteSpace: 'nowrap', textAlign: c.align || 'left', fontWeight: 700 }}>
                    {c.render(row)}
                  </td>
                ))}
                <td style={{ padding: '4px 8px', textAlign: 'center' }}>
                  <button onClick={() => handleDelete(row.id)} style={{ background: 'none', border: 'none', color: '#e57373', cursor: 'pointer', fontSize: 14 }} title="삭제">✕</button>
                </td>
              </tr>
            ))}
            {adding && (
                ...
                {offlineCols.map(c => (
                  <td key={c.key} style={{ padding: '4px 8px' }}>
                    <input
                      ...
                      onChange={e => { let v = e.target.value; setNewRow(prev => ({ ...prev, [c.key]: v })) }}
                      onBlur={e => {
                      ...
                      placeholder={c.label}
                      style={{ width: (c.width - 12) + 'px', padding: '2px 4px', fontSize: 11, background: '#0d2035', border: '1px solid #3a6090', borderRadius: 3, color: '#cce0f5', outline: 'none' }}
                    />
                  </td>
                ))}
                {CALCULATED_COLS.map(c => (
                  <td key={c.key} className={'calculated-profit ' + (c.tone ? c.tone(newRow) : '')} style={{ padding: '4px 8px', whiteSpace: 'nowrap', textAlign: c.align || 'left', fontWeight: 700 }}>
                    {c.render(newRow)}
                  </td>
                ))}
        ...
        <div style={{ marginTop: 10 }}>
          {!adding && (
            <button onClick={() => setAdding(true)} style={{ background: 'none', border: '1px dashed #4a7aaa', color: '#6ab0e0', borderRadius: 4, padding: '4px 12px', cursor: 'pointer', fontSize: 12 }}>+ 제품 추가</button>
          )}
        </div>
      </div>
    </div>
    ...
    <div style={{ overflowX: 'auto' }}>
      <table style={{ borderCollapse: 'collapse', minWidth: totalWidth, fontSize: 12, width: '100%' }}>
        <thead>
          <tr style={{ background: '#1a3050', color: '#8ab4d8' }}>
            {displayCols.map(c => (
              <th key={c.key} style={{ padding: '6px 8px', textAlign: 'left', whiteSpace: 'nowrap', width: c.width, borderBottom: '1px solid #2a4060' }}>{c.label}</th>
            ))}
            <th style={{ padding: '6px 8px', width: 60, borderBottom: '1px solid #2a4060' }}>액션</th>
          </tr>
        </thead>
...
function SkuTable({ skus, onRefresh, notify }) {
  ...
  const handleEditSave = async () => {
    ...
    try {
      ...
      cancelEdit(); onRefresh(); notify('SKU 정보가 수정되었습니다.', 'success')
    } catch { notify('수정 실패', 'error') }
  }
  ...
  return (
    <div style={{ overflowX: 'auto' }}>
      ...
      {!adding && (
        <button onClick={() => setAdding(true)} style={{ marginTop: 10, background: 'none', border: '1px dashed #4a7aaa', color: '#6ab0e0', borderRadius: 4, padding: '4px 12px', cursor: 'pointer', fontSize: 12 }}>+ SKU 추가</button>
      )}
    </div>
  )
}
  ...
  return (
    <div>
      ...
      {adding ? (
        ...
        <button onClick={() => setAdding(true)} style={{ marginTop: 8, background: 'none', border: '1px dashed #4a7aaa', color: '#6ab0e0', borderRadius: 4, padding: '4px 12px', cursor: 'pointer', fontSize: 12 }}>+ 구간 추가</button>
      )}
    </div>
  )
}
...
function buildCostOverview(data) {
  ...
  Object.entries(data.channels || {}).forEach(([channelName, channelRows]) => {
    ;(channelRows || []).forEach(row => {
      ...
      rows.push({ key, channelName, productCode: row.product_code || '', productName: row.product_name || '', skuCode: row.sku_code || '', salesBase: preview.salesBase, operatingMargin: preview.operatingMargin, operatingProfit: preview.operatingProfit })
    })
  })
  const grouped = new Map()
  rows.forEach(row => {
    const group = grouped.get(row.key) || { key: row.key, productCode: row.productCode, productName: row.productName, skuCodes: new Set(), channels: [], totalRows: 0, minMargin: null, maxMargin: null, totalOperatingProfit: 0 }
  ...
  return Array.from(grouped.values()).map(row => ({ ...row, skuCodes: Array.from(row.skuCodes), channels: Array.from(new Set(row.channels)), duplicateCount: Math.max(0, row.totalRows - 1) })).sort((a, b) => b.duplicateCount - a.duplicateCount || String(a.productName).localeCompare(String(b.productName), 'ko'))
}
          ...
          <tbody>
            {rows.map(row => (
              <tr key={row.key} style={{ borderBottom: '1px solid #e2e8f0' }}>
                ...
                <td style={{ padding: '6px 8px', textAlign: 'right', color: row.duplicateCount ? '#b45309' : '#64748b', fontWeight: 800 }}>{row.duplicateCount ? '+' + row.duplicateCount : '-'}</td>
                <td style={{ padding: '6px 8px', textAlign: 'right' }}>{row.minMargin == null ? '-' : row.minMargin === row.maxMargin ? pct(row.minMargin) : pct(row.minMargin) + ' ~ ' + pct(row.maxMargin)}</td>
                <td className={'calculated-profit ' + profitTone(row.totalOperatingProfit)} style={{ padding: '6px 8px', textAlign: 'right', fontWeight: 800 }}>{won(Math.round(row.totalOperatingProfit))}</td>
              </tr>
            ))}
          </tbody>
        </table>
...
export default function ProductCostPage() {
  ...
  return (
    <div className="product-cost-page" style={{ minHeight: '100vh', background: '#0d1b2e', color: '#c8dff0', fontFamily: 'sans-serif' }}>
      ...
      {message.text && (
        <div style={{ background: msgBg[message.type] || '#1a3050', border: '1px solid ' + (message.type === 'success' ? '#2a6a3a' : message.type === 'error' ? '#6a2a2a' : '#2a5070'), borderRadius: 6, padding: '8px 14px', marginBottom: 16, fontSize: 13, color: message.type === 'success' ? '#6ee89a' : message.type === 'error' ? '#f08080' : '#8ab4d8' }}>
          {message.text}
        </div>
      )}
      ...
      <div className="product-cost-tabs" style={{ display: 'flex', gap: 2, marginBottom: 16, borderBottom: '1px solid #1e3050', flexWrap: 'wrap' }}>
        {tabs.map(t => (
          <button key={t} onClick={() => setActiveTab(t)}
            ...
            style={{ background: activeTab === t ? '#1a4070' : 'none', color: activeTab === t ? '#7bc8ff' : '#6a9abf', border: 'none', borderBottom: activeTab === t ? '2px solid #4a8fcf' : '2px solid transparent', padding: '8px 14px', cursor: 'pointer', fontSize: 13, fontWeight: activeTab === t ? 600 : 400, borderRadius: '4px 4px 0 0' }}>
            {t}
            {CHANNELS.includes(t) && data.channels[t] && (
              <span style={{ marginLeft: 5, fontSize: 10, color: '#4a8fcf' }}>({(data.channels[t] || []).length})</span>
            )}
            {t === 'SKU마스터' && (data.skuMaster || []).some(row => !String(row.product_name || '').trim()) && (
              <span style={{ marginLeft: 5, fontSize: 10, color: '#dc2626' }}>({(data.skuMaster || []).filter(row => !String(row.product_name || '').trim()).length}명 필요)</span>
            )}
          </button>
        ))}
      </div>
      ...
      {loading ? (
        ...
        <div className="product-cost-content" style={{ background: '#111e2e', borderRadius: 8, padding: 16 }}>
          ...
          {activeTab === '오프라인(납품처별)' && (
            <OfflineTable
              rows={data.channels['오프라인(납품처별)'] || []}
              onRefresh={load}
              notify={notify}
            />
          )}
          {CHANNELS.filter(c => c !== '오프라인(납품처별)').includes(activeTab) && (
            <ChannelTable channelName={activeTab} rows={data.channels[activeTab] || []} onRefresh={load} notify={notify} />
          )}
      ...
      <div className="product-cost-formula" style={{ marginTop: 20, background: '#0a1828', borderRadius: 8, padding: 16, fontSize: 12, color: '#6a9abf', border: '1px solid #1a3050' }}>
        ...
        <div style={{ marginTop: 6, color: '#4a7a9a' }}>채널별 매출 페이지에서 상품코드를 기반으로 자동 조인되어 계산됩니다.</div>
      </div>
    </div>
  )
}
