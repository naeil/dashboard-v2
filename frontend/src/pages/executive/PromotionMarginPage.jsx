...
function matchSalesProduct(costRow, products) {
  ...
  const names = [
    costRow.product_name,
    costRow.product_code,
    costRow.sku_code,
  ...
  return (
    <div className="flex items-center rounded border border-slate-300 bg-white px-3">
      <input
        ...
        className="h-10 w-full bg-transparent text-sm font-black text-slate-950 outline-none"
        inputMode="decimal"
      />
      ...
      <header className="flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <p className="mt-2 text-sm font-bold text-slate-600">
            MD가 할인, 1+1, 오프라인 행사를 설계할 때 원가와 실시간 판매 흐름 기준으로 손익을 바로 확인합니다.
          </p>
        </div>
        <div className="flex flex-wrap items-center gap-2">
          ...
          <button onClick={load} className="h-10 rounded border border-slate-300 bg-white px-4 text-sm font-black text-slate-700">
            조회
          </button>
          <button onClick={refreshSales} disabled={syncing} className="h-10 rounded bg-blue-600 px-4 text-sm font-black text-white disabled:opacity-50">
            {syncing ? '갱신 중' : '실시간 판매 업데이트'}
          </button>
        </div>
      </header>

      {message && (
        <div className="rounded border border-blue-200 bg-blue-50 px-4 py-3 text-sm font-black text-blue-700">{message}</div>
      )}

      <section className="grid grid-cols-1 gap-4 lg:grid-cols-4">
        <div className="rounded border border-slate-200 bg-white p-5 shadow-sm">
          <p className="text-xs font-black text-slate-500">기간 실시간 매출</p>
          <strong className="mt-4 block text-2xl font-black">{won(analytics.summary?.salesAmount)}</strong>
          <p className="mt-2 text-xs font-bold text-slate-500">PlayAuto 주문 현황 기준</p>
        </div>
        <div className="rounded border border-slate-200 bg-white p-5 shadow-sm">
          <p className="text-xs font-black text-slate-500">기간 주문수</p>
          <strong className="mt-4 block text-2xl font-black">{count(analytics.summary?.orderCount)}건</strong>
          <p className="mt-2 text-xs font-bold text-slate-500">선택 기간 합계</p>
        </div>
        <div className="rounded border border-slate-200 bg-white p-5 shadow-sm">
          <p className="text-xs font-black text-slate-500">예상 행사 매출</p>
          <strong className="mt-4 block text-2xl font-black">{won(result.revenue)}</strong>
          <p className="mt-2 text-xs font-bold text-slate-500">{count(result.expectedOrders)}건 기준</p>
        </div>
        <div className={`rounded border p-5 shadow-sm ${decision.className}`}>
          <p className="text-xs font-black">행사 판단</p>
          <strong className="mt-4 block text-2xl font-black">{decision.label}</strong>
          <p className="mt-2 text-xs font-black">영업이익률 {pct(result.operatingMargin)}</p>
        </div>
      </section>
      ...
      <section className="grid grid-cols-1 gap-5 xl:grid-cols-[420px_1fr]">
        <aside className="rounded border border-slate-200 bg-white p-5 shadow-sm">
          ...
          <div className="mt-4 space-y-4">
            <Field label="상품 검색">
              <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="상품명, SKU, 코드 검색" className="h-10 w-full rounded border border-slate-300 px-3 text-sm font-bold outline-none" />
            </Field>
            <Field label="상품 선택">
              <select value={selectedRow?.rowKey || ''} onChange={(e) => setSelectedRowKey(e.target.value)} className="h-11 w-full rounded border border-slate-300 px-3 text-sm font-black outline-none">
                {filteredCostRows.map((row) => (
                  <option key={row.rowKey} value={row.rowKey}>{productLabel(row)}</option>
                ))}
              </select>
            </Field>
            <Field label="행사명">
              <input value={form.promoName} onChange={(e) => updateForm('promoName', e.target.value)} placeholder="예: 쿠팡 1+1 주말 특가" className="h-10 w-full rounded border border-slate-300 px-3 text-sm font-bold outline-none" />
            </Field>
            <div className="grid grid-cols-2 gap-3">
              ...
              <Field label="행사 유형">
                <select value={form.promoType} onChange={(e) => updateForm('promoType', e.target.value)} className="h-10 w-full rounded border border-slate-300 px-3 text-sm font-black outline-none">
                  {promoTypes.map((item) => <option key={item.id} value={item.id}>{item.label}</option>)}
                </select>
              </Field>
            </div>
            <div className="grid grid-cols-2 gap-3">
        ...
        <main className="space-y-5">
          <section className="rounded border border-slate-200 bg-white p-5 shadow-sm">
            <div className="flex flex-wrap items-center justify-between gap-3">
              <div>
                <p className="text-xs font-black text-slate-500">선택 상품</p>
                <h2 className="mt-1 text-xl font-black">{selectedRow?.product_name || '상품을 선택하세요'}</h2>
                <p className="mt-1 text-xs font-bold text-slate-500">{selectedRow?.channelName || '-'} / SKU {selectedRow?.sku_code || '-'}</p>
              </div>
              ...
              <table className="w-full min-w-[760px] border-collapse text-sm">
                <thead>
                  <tr className="border-b bg-slate-50 text-left text-xs font-black text-slate-500">
                    ...
                    <th className="p-3">기준</th>
                    <th className="p-3 text-right">매출 대비</th>
                  </tr>
                </thead>
                <tbody className="font-bold">
                  {[
                    ['행사 매출', result.revenue, `${count(result.expectedOrders)}건 x ${won(form.promoPrice)}`, 100],
                    ['제품 원가', result.productionCost, `${count(result.unitsPerOrder)}개 구성`, result.revenue ? (result.productionCost / result.revenue) * 100 : 0],
                    ['물류비', result.logisticsCost, '주문당 물류비', result.revenue ? (result.logisticsCost / result.revenue) * 100 : 0],
                    ['채널 수수료', result.channelFee, `${form.channelFeeRate}%`, result.revenue ? (result.channelFee / result.revenue) * 100 : 0],
                    ['광고비', result.adCost, `${form.adRate}%`, result.revenue ? (result.adCost / result.revenue) * 100 : 0],
                    ['마케팅비', result.marketingCost, `${form.marketingRate}%`, result.revenue ? (result.marketingCost / result.revenue) * 100 : 0],
                    ['판관비', result.opexCost, `${form.opexRate}%`, result.revenue ? (result.opexCost / result.revenue) * 100 : 0],
                    ['쿠폰/지원금', result.supportCost, '개당 지원금', result.revenue ? (result.supportCost / result.revenue) * 100 : 0],
                    ['행사 고정비', result.fixedEventCost, '촬영, 매대, 배너 등', result.revenue ? (result.fixedEventCost / result.revenue) * 100 : 0],
                  ].map(([label, amount, basis, ratio]) => (
                    <tr key={label} className="border-b border-slate-100">
                      <td className="p-3">{label}</td>
                      <td className="p-3 text-right">{won(amount)}</td>
                      <td className="p-3 text-slate-500">{basis}</td>
                      <td className="p-3 text-right">{pct(ratio)}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
          ...
          <section className="grid grid-cols-1 gap-5 lg:grid-cols-2">
            <div className="rounded border border-slate-200 bg-white p-5 shadow-sm">
              ...
              <p className="mt-3 text-xs font-bold text-slate-500">
                상품명, 상품코드, SKU가 판매 데이터와 일치하면 자동 매칭됩니다.
              </p>
            </div>
            <div className="rounded border border-slate-200 bg-white p-5 shadow-sm">
              ...
              <div className="mt-4 max-h-56 space-y-2 overflow-auto">
                {plans.length === 0 ? (
                  <p className="rounded bg-slate-50 p-4 text-sm font-bold text-slate-500">저장된 서식이 없습니다.</p>
                ) : plans.map((plan) => (
                  <div key={plan.id} className="rounded border border-slate-200 p-3">
                    <div className="flex items-center justify-between gap-3">
                      <b className="text-sm">{plan.promoName}</b>
                      <button onClick={() => setPlans((prev) => prev.filter((item) => item.id !== plan.id))} className="text-xs font-black text-rose-600">삭제</button>
                    </div>
                    <p className="mt-1 text-xs font-bold text-slate-500">{plan.productName} / {plan.channel} / {plan.promoType}</p>
                    <p className="mt-2 text-xs font-black">매출 {won(plan.revenue)} · 이익 {won(plan.operatingProfit)} · 이익률 {pct(plan.operatingMargin)}</p>
                  </div>
                ))}
              </div>
            </div>
          ...
          <section className="rounded border border-slate-200 bg-white p-5 shadow-sm">
            <h2 className="text-xl font-black">최근 판매 상품 Top 10</h2>
              ...
              <table className="w-full min-w-[780px] border-collapse text-sm">
                <thead>
                  <tr className="border-b bg-slate-50 text-left text-xs font-black text-slate-500">
                    ...
                    <th className="p-3 text-right">주문</th>
                    <th className="p-3 text-right">추정 영업이익</th>
                    <th className="p-3 text-right">추정 이익률</th>
                  </tr>
                </thead>
                <tbody className="font-bold">
                  {(analytics.products || []).slice(0, 10).map((row, index) => {
                    const profit = row.has_actual_cost ? row.actual_operating_profit : row.estimated_operating_profit
                    const margin = row.has_actual_cost ? row.actual_operating_margin : row.estimated_operating_margin
                    return (
                      <tr key={`${row.product_name || row.productName}-${index}`} className="border-b border-slate-100">
                        <td className="p-3">{row.product_name || row.productName || '-'}</td>
                        <td className="p-3 text-right">{won(row.sales_amount)}</td>
                        <td className="p-3 text-right">{count(row.order_count)}건</td>
                        <td className={`p-3 text-right ${num(profit) >= 0 ? 'text-blue-700' : 'text-rose-600'}`}>{won(profit)}</td>
                        <td className="p-3 text-right">{pct(Number(margin || 0) * (Math.abs(Number(margin || 0)) <= 1 ? 100 : 1))}</td>
                      </tr>
                    )
                  })}
                </tbody>
