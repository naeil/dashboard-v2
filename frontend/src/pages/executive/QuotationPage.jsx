...
export default function QuotationPage() {
  ...
  const loadLibs = () => {
    const libs = [
      ...
      { id: 'jspdf-auto', src: 'https://cdnjs.cloudflare.com/ajax/libs/jspdf-autotable/3.8.2/jspdf.plugin.autotable.min.js' },
  ...
  const handleSavePDF = async () => {
    await loadLibs()
    if (!window.jspdf) { alert('PDF 라이브러리 로딩 실패. 잠시 후 다시 시도해주세요.'); return }
    ...
    try {
      ...
      if (img.complete && img.naturalWidth > 0) {
        ...
        doc.addImage(canvas.toDataURL('image/png'), 'PNG', 75, 6, 60, 18)
      }
    } catch {}
    doc.setFontSize(18); doc.setFont('helvetica', 'bold')
    doc.text('QUOTATION (Gyeonjeokseo)', 105, 32, { align: 'center' })
    doc.setDrawColor(180); doc.setLineWidth(0.5); doc.line(15, 36, 195, 36)
    doc.setFontSize(9); doc.setFont('helvetica', 'bold'); doc.text('CLIENT', 15, 42)
    doc.setFont('helvetica', 'normal')
    doc.text('Name: ' + (form.clientName || '-'), 15, 48)
    doc.text('Contact: ' + (form.clientContact || '-'), 15, 54)
    doc.setFont('helvetica', 'bold'); doc.text('QUOTATION INFO', 120, 42)
    doc.setFont('helvetica', 'normal')
    doc.text('No: ' + (form.qNumber || '-'), 120, 48)
    doc.text('Date: ' + (form.qDate || '-'), 120, 54)
    doc.text('Valid: ' + (form.qValid || '-'), 120, 60)
    doc.text('Delivery: ' + (form.qDelivery || '-'), 120, 66)
    doc.setFont('helvetica', 'bold'); doc.text('SUPPLIER', 15, 62)
    doc.setFont('helvetica', 'normal')
    doc.text(form.issuerName || '-', 15, 68)
    doc.text((form.issuerContact || '') + '  ' + (form.issuerPhone || ''), 15, 74)
    doc.line(15, 78, 195, 78)
    const tableRows = rows.map((r, i) => {
      const s = r.qty * r.price; const v = Math.round(s * 0.1)
      return [String(i + 1), r.name, r.spec, String(r.qty), r.price.toLocaleString(), s.toLocaleString() + '원', v.toLocaleString() + '원', (s + v).toLocaleString() + '원']
    })
    doc.autoTable({ startY: 82, head: [['#', 'Item', 'Spec', 'Qty', 'Unit Price', 'Supply', 'VAT', 'Total']], body: tableRows, theme: 'grid', headStyles: { fillColor: [14, 165, 233], textColor: 255, fontStyle: 'bold', fontSize: 8.5 }, bodyStyles: { fontSize: 8 }, columnStyles: { 0: { halign: 'center', cellWidth: 8 }, 3: { halign: 'center', cellWidth: 12 }, 4: { halign: 'right', cellWidth: 22 }, 5: { halign: 'right', cellWidth: 26 }, 6: { halign: 'right', cellWidth: 18 }, 7: { halign: 'right', cellWidth: 26 } }, margin: { left: 15, right: 15 } })
    const fy = doc.lastAutoTable.finalY + 6
    doc.setFontSize(9); doc.setFont('helvetica', 'normal')
    doc.text('Supply Total:', 130, fy + 6); doc.text(fmt(subtotal), 192, fy + 6, { align: 'right' })
    doc.text('VAT Total:', 130, fy + 12); doc.text(fmt(vatTotal), 192, fy + 12, { align: 'right' })
    doc.setFont('helvetica', 'bold'); doc.setFontSize(11); doc.text('GRAND TOTAL:', 125, fy + 20)
    doc.setTextColor(14, 165, 233); doc.text(fmt(grandTotal), 192, fy + 20, { align: 'right' })
    doc.setTextColor(0); doc.setFont('helvetica', 'normal'); doc.setFontSize(8)
    doc.setTextColor(150); doc.text('Generated by NAEIL GROUP Business Platform', 105, 290, { align: 'center' })
    doc.save('견적서_' + (form.qNumber || 'Q') + '_' + form.qDate + '.pdf')
  }
  ...
  const handleSaveExcel = async () => {
    ...
    const d = [
      ...
      ...rows.map((r, i) => { const s = r.qty * r.price; const v = Math.round(s * 0.1); return [i + 1, r.name, r.spec, r.qty, r.price, s, v, s + v] }),
    ...
    ws['!cols'] = [{ wch: 8 }, { wch: 24 }, { wch: 14 }, { wch: 8 }, { wch: 18 }, { wch: 16 }, { wch: 14 }, { wch: 16 }]
    window.XLSX.utils.book_append_sheet(wb, ws, '견적서')
    window.XLSX.writeFile(wb, '견적서_' + (form.qNumber || 'Q') + '_' + form.qDate + '.xlsx')
  }
