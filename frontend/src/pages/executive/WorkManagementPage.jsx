  ...
  return (
    <>
      <div className="flex flex-col gap-4 xl:flex-row xl:items-end xl:justify-between">
        ...
        <button type="button" onClick={load} className="mb-6 h-11 rounded-lg border border-white/10 px-5 text-sm font-black text-slate-200 hover:bg-white/5">
      ...
      <div className="mb-6 flex flex-col gap-3 rounded-lg border border-white/10 bg-slate-900/70 p-5 xl:flex-row xl:items-center xl:justify-between">
        <div>
          <h2 className="text-lg font-black text-white">전체 업무 상황판</h2>
          <p className="mt-1 text-xs font-bold text-slate-400">위험 업무, 막힌 업무, 승인 대기 업무를 먼저 확인합니다.</p>
        </div>
        <div className="flex flex-wrap gap-2">
          {assignees.map((name) => (
            <button
              ...
              className={`h-10 rounded-lg border px-4 text-sm font-black transition-colors ${selectedAssignee === name ? 'border-sky-400/40 bg-sky-400/15 text-sky-100' : 'border-white/10 bg-slate-950 text-slate-400 hover:bg-white/5'}`}
            >
              {name}
            </button>
          ))}
      ...
      <section className="mb-6 grid grid-cols-1 gap-6 xl:grid-cols-2">
        <Panel title="직원별 업무 현황">
          <DataTable
            ...
            searchable={false}
            columns={[
              { key: 'assignee', label: '담당자', render: (row) => <span className="font-black text-white">{row.assignee}</span> },
              { key: 'active', label: '진행', render: (row) => count(row.active, '건') },
              { key: 'delayed', label: '지연', render: (row) => <span className={row.delayed > 0 ? 'font-black text-rose-200' : 'text-slate-300'}>{count(row.delayed, '건')}</span> },
              { key: 'blocked', label: '막힘', render: (row) => <span className={row.blocked > 0 ? 'font-black text-rose-200' : 'text-slate-300'}>{count(row.blocked, '건')}</span> },
              { key: 'review', label: '승인대기', render: (row) => count(row.review, '건') },
              { key: 'avgProgress', label: '평균 진행률', render: (row) => (
                <div className="min-w-28">
                  <p className="mb-1 text-xs font-black text-slate-300">{row.avgProgress}%</p>
        ...
        <Panel title="프로젝트별 진행률">
          <DataTable
            ...
            searchable={false}
            columns={[
              { key: 'project_name', label: '프로젝트', render: (row) => <span className="font-black text-white">{row.project_name}</span> },
      ...
      <Panel title="위험 / 승인 대기 / 피드백 업무" right={<span className="text-xs font-black text-slate-400">{riskRows.length}건</span>}>
        <DataTable
          rows={riskRows}
          rowKey={(row) => row.id}
          columns={[
            ...
            { key: 'review_comment', label: '상사 피드백', render: (row) => (
              <div className="min-w-80">
                <textarea
                  ...
                  rows="2"
                  className="w-full rounded-lg border border-white/10 bg-slate-950 px-3 py-2 text-xs font-bold text-white outline-none focus:border-sky-400"
                  placeholder="직원에게 전달할 피드백"
                />
                <div className="mt-2 flex gap-2">
                  <button type="button" onClick={() => saveFeedback(row)} className="h-8 rounded-md border border-sky-400/30 bg-sky-400/10 px-3 text-xs font-black text-sky-100 hover:bg-sky-400/20">
                    피드백 전달
                  </button>
                  {(row.status === 'REVIEW' || row.approval_required) && (
                    <button type="button" onClick={() => approveTask(row)} className="h-8 rounded-md border border-emerald-400/30 bg-emerald-400/10 px-3 text-xs font-black text-emerald-100 hover:bg-emerald-400/20">
                      완료 승인
                    </button>
                  )}
                </div>
